...
import { createNoteLabel, updateNoteLabel, deleteNoteLabel } from '@/app/admin/actions'
...
export function NoteLabelsList({ labels: initial }: { labels: NoteLabel[] }) {
  const [labels, setLabels] = useState<NoteLabel[]>(initial)
  ...
  const handleCreate = () => {
    ...
    startTransition(async () => {
      ...
      const result = await createNoteLabel(newName.trim(), newColor, maxOrder + 1)
      if (result.error) {
        ...
        setNewColor('#3b82f6')
        setError(null)
        // Optimistic: reload will happen via revalidatePath
        setLabels(prev => [...prev, {
  ...
  const handleToggleActive = (label: NoteLabel) => {
    setLabels(prev => prev.map(l => l.id === label.id ? { ...l, active: !l.active } : l))
    startTransition(async () => {
      await updateNoteLabel(label.id, { active: !label.active })
    })
  }

  const handleRename = (label: NoteLabel, name: string) => {
    setLabels(prev => prev.map(l => l.id === label.id ? { ...l, name } : l))
  }

  const handleRenameSave = (label: NoteLabel) => {
    startTransition(async () => {
      await updateNoteLabel(label.id, { name: label.name })
    })
  }

  const handleColorChange = (label: NoteLabel, color: string) => {
    setLabels(prev => prev.map(l => l.id === label.id ? { ...l, color } : l))
    startTransition(async () => {
      await updateNoteLabel(label.id, { color })
    })
  }

  const handleDelete = (label: NoteLabel) => {
    if (!confirm(`Label "${label.name}" verwijderen?`)) return
    setLabels(prev => prev.filter(l => l.id !== label.id))
    startTransition(async () => {
      await deleteNoteLabel(label.id)
    })
  }
          ...
          <div key={label.id} className="flex items-center gap-3 py-2 border-b last:border-0">
            ...
            <div className="flex items-center gap-1.5 ml-auto shrink-0">
              <Switch
                ...
                id={`active-${label.id}`}
              />
              <Label htmlFor={`active-${label.id}`} className="text-xs text-muted-foreground cursor-pointer">
