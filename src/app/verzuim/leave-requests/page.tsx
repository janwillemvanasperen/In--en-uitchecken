...
export default async function VerzuimLeaveRequestsPage() {
  await requireVerzuim()
  const supabase = await createClient()
  const adminClient = createAdminClient()

  const [{ data: leaveRequests }, { data: coaches }, { data: usersData }] = await Promise.all([
    adminClient.from('leave_requests').select('*, users!leave_requests_user_id_fkey(full_name)').order('created_at', { ascending: false }),
