...
export default async function SchedulePushPage() {
  await requireAdmin()
  const adminClient = createAdminClient()

  const [{ data: students }, { data: pushRequests }] = await Promise.all([
    // All students with coach name
    adminClient
      .from('users')
    ...
    // Push history (newest first, max 20)
    adminClient
                ...
                <CardHeader className="pb-2">
                  <CardTitle className="text-sm flex items-center gap-2">
                    ...
                    <Badge
                      ...
                      className={allDone ? 'bg-green-100 text-green-800 ml-auto' : 'ml-auto'}
                    >
